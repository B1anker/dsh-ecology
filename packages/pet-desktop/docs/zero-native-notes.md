...
5. **SDK 参考渲染器丢弃负缩放（2026-09-05 Windows 实机确认；app 侧已绕过）**：`SDK/src/primitives/canvas/reference.zig` 的 `drawImage`（`:491` 起）把 `Affine` 经 `transform.transformRect(...).normalized()` 折叠成轴对齐包围盒——平移与缩放模长保留，**负缩放的符号被丢弃**。Windows 透明窗口（WS_EX_LAYERED）恒走这条 CPU 参考渲染路径（任何软件回退亦然），于是 `{ .a = -1, .tx = window_size }` 的镜像变换在 macOS/Metal 正常、在 Windows 上失效（向右拖拽时精灵仍朝左的实机 bug 即此根因）。SDK 是锁定 commit 的 tarball 依赖、不可打补丁，**绕过方案在 app 侧**：为 `working` 条带预生成逐帧水平镜像的 `working-mirrored.png`（`scripts/mirror-working-strips.mjs`，manifest 增加 `mirroredFile` 字段，镜像图占每 pet 16 槽 id 步长的第 8 槽 `manifest.mirroredRunImageId`），朝右拖拽时 `model.sprite()` 直接换用镜像图 id、不再施加负缩放 Affine；无 `mirroredFile` 的导入宠物回退旧 Affine（Metal 行为不变，参考渲染器下维持不翻转的现状）。值得向框架上报：参考渲染器应支持逐像素反向采样而非 bbox 归一化。
