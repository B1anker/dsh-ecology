...
2. **Windows 精灵图路径**：透明窗口在 Windows 上拒绝 WebView，必须走 gpu_surface——spike 已是此路线，天然兼容；需验证 Win32 下 `mouseLocation`/`pressedMouseButtons` 的等价物（`GetCursorPos` / `GetAsyncKeyState(VK_LBUTTON)`）与逐像素透明（layered window）在框架宿主侧的接线。
