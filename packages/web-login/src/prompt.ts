 ...
 * cannot be checked by hand without leaking the password into scrollback. Their
 * types are structural for the same reason: requiring a real `ReadStream` would
 * make the fake impossible to write, and this module uses five members of it.
 *
 * @module @seaveyon/dsh-web-login/prompt
 */

/** The input surface the prompt needs: a TTY it can put into raw mode. */
...
/** The output surface the prompt needs: somewhere to put the prompt text. */
export interface PromptOutput {
  write: (chunk: string) => unknown
}

/** Streams to read from and write to; both default to the process TTY. */
...
export function askHidden(
  ...
  if (input.isTTY !== true) {
    return Promise.reject(
      new Error('dsh-web-login: this command must run in an interactive terminal'),
    )
  }
