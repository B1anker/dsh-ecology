...
import { fakeRequest, fakeResponse, fakeStreamingRequest } from '../helpers/fake-http.js'
...
test('clientKey uses the socket address and ignores a forged header by default', () => {
  ...
  const options: ClientKeyOptions = { trustProxy: false, clientIpHeader: 'x-forwarded-for' }
  expect(clientKey(req, options)).toBe('10.0.0.7')
})
...
test('clientKey takes the last forwarded hop when the proxy is trusted', () => {
  ...
  const options: ClientKeyOptions = { trustProxy: true, clientIpHeader: 'x-forwarded-for' }
  expect(clientKey(req, options)).toBe('203.0.113.5')
})

test('clientKey falls back to the socket when the trusted header is missing or blank', () => {
  const options: ClientKeyOptions = { trustProxy: true, clientIpHeader: 'x-forwarded-for' }
  expect(clientKey(fakeRequest({ remoteAddress: '10.0.0.7' }), options)).toBe('10.0.0.7')
  const blank = fakeRequest({
    headers: { 'x-forwarded-for': '  ,  ' },
    remoteAddress: '10.0.0.7',
  })
  expect(clientKey(blank, options)).toBe('10.0.0.7')
})
...
test('clientKey handles a repeated header and a socket with no address', () => {
  ...
  expect(clientKey(repeated, options)).toBe('1.1.1.1')
  ...
  expect(clientKey(noSocket, { trustProxy: false, clientIpHeader: 'x-forwarded-for' })).toBe(
    'unknown',
  )
})
