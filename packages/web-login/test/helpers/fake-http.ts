 ...
 * `as unknown as IncomingMessage` beside every assertion and bury the thing
 * each test is actually about. The cast is sound in exactly one direction: the
 * fakes implement the members these helpers touch, and nothing more.
 *
...
export function fakeRequest({
  ...
  // absent address: that is the shape a request has after the peer has gone,
  // and `clientKey` has to survive it without throwing.
 ...
 *
 * `fakeRequest` is enough wherever a fixed body that ends is enough. This exists
 * for the three properties that are only observable mid-flight: that `readBody`
 * does not destroy a request it refuses (destroying an `IncomingMessage` tears
 * down its socket, so the 413 never arrives), that it removes its listeners once
 * it has settled, and that it resolves rather than rejects when the client
