...
import { createDecorator, type ServiceIdentifier } from './identifier.js'
...
export const IPluginContext = createDecorator<HostContext>('pluginContext')
export const IWebServer = createDecorator<HostWebServer>('webServer')
export const IConnection = createDecorator<HostConnection>('connection')
export const ITools = createDecorator<HostTools>('tools')
 ...
 */
export function registerHostServices(
  ...
  for (const id of services.required ?? []) {
    const service = ctx.get(String(id))
    if (service === undefined) {
      throw new Error(`Host service '${String(id)}' is not available on this plugin context`)
    }
    collection.set(id, service)
  }
  for (const id of services.optional ?? []) {
    const service = ctx.get(String(id))
    if (service !== undefined) collection.set(id, service)
  }
  return collection
}
